...
def get_weekly_report_record(report_name,filters):
    ...
    if filters.cost_center:	
        ...
        #            and MONTH(s.transaction_date) != MONTH(a.posting_date)
        #            or ((MONTH(s.transaction_date) = MONTH(a.posting_date)) and (YEAR(s.transaction_date) != YEAR(a.posting_date)))
    ...
    ##for dd in min_date_backloglst:		
        ...
        ##year_total_list.setdefault(dd[3], frappe._dict()).setdefault(dd[1],frappe._dict()).setdefault(dd[0],dd[2])
        ##year_total_list[dd[3]][dd[1]][dd[0]] += flt(dd[2])	
    ...
    #gross_profit_data = GrossProfitGeneratorforweeklyrpt(filters)
    #print(gross_profit_data.si_list_prevyrs)
    #columns = get_columns(filters)
    #data = get_merged_dataongrossprofls(filters,gross_profit_data.si_list)

    return Cust_colum_name,combined_list,compnyName,fiscal_endDt	
...
def getsalesbacklogforweeksql(week_end_date):
    retsql = """
            select 'Consolidated' as entity, i.base_amount as value_field, s.transaction_date, s.company, '{to_date}' as endofweekdate, WEEK('{to_date}') as weekno,a.posting_date as adeliverydate from `tabSales Order` s inner join  
            `tabSales Order Item` i on s.name = i.parent left join `tabDelivery Note Item` b on s.name = b.against_sales_order and b.so_detail = i.name inner join `tabDelivery Note` a on b.parent = a.name where s.transaction_date != a.posting_date and s.status <> 'Cancelled'
            and s.transaction_date <= '{to_date}' 			
            and ((a.posting_date > '{to_date}') or (a.posting_date IS NULL))
            UNION
            select s.cost_center as entity, i.base_amount as value_field, s.transaction_date, s.company, '{to_date}' as endofweekdate, WEEK('{to_date}') as weekno,a.posting_date as adeliverydate
            from `tabSales Order` s inner join `tabSales Order Item` i 
             on s.name = i.parent left join `tabDelivery Note Item` b on 
            s.name = b.against_sales_order and b.so_detail = i.name inner join `tabDelivery Note` a 
            on b.parent = a.name 
            where s.transaction_date <= '{to_date}' 
            and s.status <> 'Cancelled' 
            and ((a.posting_date > '{to_date}') or (a.posting_date IS NULL))
            
            """.format(to_date = week_end_date)
      
    return retsql    

def getsalesbacklogforweekallsql(week_end_date):
    allretsql = ""
    i = 0
    while(i < 53):
        if (i == 0):
            allretsql += getsalesbacklogforweeksql(week_end_date)
        else:
            allretsql += "UNION " + getsalesbacklogforweeksql(week_end_date)		
        i += 1

    ststr = "select X.* from ( "
    endstr = ") X"
    fullsqlstr = ststr + allretsql + endstr
...
def get_weeklysalesprofit_report_record(report_name,filters):
    from dateutil.relativedelta import MO, relativedelta
    # Skipping total row for tree-view reports
    skip_total_row = 0
    #return self.columns, self.data, None, None, skip_total_row
...
class GrossProfitGeneratorforweeklyrpt(object):
    def __init__(self, filters=None):
        self.data = []
    ...
    def load_invoice_items(self):
        ...
        if self.filters.cost_center:
            ...
            conditions2 += " and cost_center = %(cost_center)s"    
        ...
        fiscalyeardtprev, prevyrsstartdate = fetch5yrsback_fiscalyear(5,self.filters)

        conditions += " and (posting_date >= '" + start_date + "' and posting_date <= '" + fiscal_endDt + "')"
        conditions2 += " and (posting_date >= '" + prevyrsstartdate.strftime('%Y-%m-%d') + "' and posting_date < '" + start_date + "')"
        #if self.filters.to_date:
        #    long_month_name = self.filters.get("month")
        #    datetime_object = datetime.datetime.strptime(long_month_name, "%B")
        #    month_number = datetime_object.month
        #    conditions += " and (posting_date >= %(from_date)s and posting_date <= %(to_date)s)"
        ...
        self.si_list_prevyrs = frappe.db.sql(
            """
            select
                `tabSales Invoice Item`.parenttype, `tabSales Invoice Item`.parent,
                `tabSales Invoice`.posting_date, `tabSales Invoice`.posting_time,
                ...
                `tabSales Invoice Item`.name as "item_row", `tabSales Invoice`.is_return,
                `tabSales Invoice Item`.cost_center,
                CONCAT(DATE_FORMAT(`tabSales Invoice`.posting_date, '%%b'),"-", RIGHT(fy.year,2)) as Date,fy.year as year
            from
                `tabSales Invoice` inner join `tabSales Invoice Item`
                    on `tabSales Invoice Item`.parent = `tabSales Invoice`.name
                inner join `tabFiscal Year` fy on `tabSales Invoice`.posting_date >= fy.year_start_date and `tabSales Invoice`.posting_date <= fy.year_end_date
            where
                `tabSales Invoice`.docstatus=1 and `tabSales Invoice`.is_opening!='Yes' {conditions} {match_cond}
            order by
                `tabSales Invoice`.posting_date desc, `tabSales Invoice`.posting_time desc""".format(
